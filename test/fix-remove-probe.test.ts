...
describe('the probe is off the child path (fix-remove-probe §2, §5)', () => {
  ...
  it('a new child reaches a real practice problem with zero intervening probe items', () => {
    ...
    // A real item was produced — a typed problem (digit/emoji prompt) OR a recognition rung
    // (whose string prompt is empty because its content is the tapped picture + options) —
    // and NO probe intervened to reach it. The itemId proves a genuine pending item exists.
    expect(it.itemId.length).toBeGreaterThan(0);
