...
function finalize(s: SprintSession, now: number): SprintResult {
  ...
  const aim = aimFor(repo.latestToolRate(s.playerId), seedGradeFor(s.schoolYear), s.skillCode, repo.bestObservedDigitRate(s.playerId));
...
export function ingestSprint(
  ...
  const aim = aimFor(repo.latestToolRate(playerId), seedGradeFor(schoolYearOf(playerId)), code, repo.bestObservedDigitRate(playerId));
...
export function chartForSkill(playerId: string, code: string): ChartData {
  ...
  return { code, points, aim: aimFor(repo.latestToolRate(playerId), seedGradeFor(schoolYearOf(playerId)), code, repo.bestObservedDigitRate(playerId)), celeration: celeration(points) };
}
