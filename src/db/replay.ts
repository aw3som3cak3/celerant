...
export function runOneOffPlacements(db: ReturnType<typeof getDb>): void {
  ...
  // Seed the Phase-B colour rung (en_color) into every existing player's ability cache — same reason.
  if (!done('bridge_english_color_v1')) {
    for (const { id } of db.prepare('SELECT id FROM player').all() as { id: string }[]) replayOne(db, id);
    mark('bridge_english_color_v1');
  }

  // Seed the Phase-C action-verb rung (en_verb_action) into every existing player's ability cache.
  if (!done('bridge_english_verb_v1')) {
    for (const { id } of db.prepare('SELECT id FROM player').all() as { id: string }[]) replayOne(db, id);
    mark('bridge_english_verb_v1');
  }

  // Seed the Phase-C -ing rung (en_verb_ing).
  if (!done('bridge_english_ing_v1')) {
    for (const { id } of db.prepare('SELECT id FROM player').all() as { id: string }[]) replayOne(db, id);
    mark('bridge_english_ing_v1');
  }

  // Seed the Phase-B attribute rung (en_attribute).
  if (!done('bridge_english_attr_v1')) {
    for (const { id } of db.prepare('SELECT id FROM player').all() as { id: string }[]) replayOne(db, id);
  ...
  // Seed the print-bridge rungs (en_word_recognise, en_word_picture) + the re-parented en_ed_regular.
  if (!done('bridge_english_print_v1')) {
    for (const { id } of db.prepare('SELECT id FROM player').all() as { id: string }[]) replayOne(db, id);
  ...
  // Seed the sentence-mode collocation/lexical rungs (en_preposition, en_is_get, en_make_do,
  // en_false_friend). Same reason; replay-all, runs once.
  if (!done('bridge_english_sentence_lex_v1')) {
    for (const { id } of db.prepare('SELECT id FROM player').all() as { id: string }[]) replayOne(db, id);
    mark('bridge_english_sentence_lex_v1');
  }
